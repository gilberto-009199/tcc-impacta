...
class MsgPiece(Msg):
    ...
    def parsePacket(self, packet):
        try:
            ...
            logger.info(
                f"Erro ao criar pacote parsePacket : "
                f"{arquivo}:{linha}, linha: {linha}, e: {e}"
            )
    ...
    def toPacket(self):
        try:
            ...
            buffer = self.buffer if isinstance(self.buffer, bytes) else self.buffer.encode('utf-8')
            ...
            logger.info(
                f"Erro ao criar pacote toPacket : "
                f"{arquivo}:{linha}, linha: {linha}, e: {e}"
            )

    @staticmethod
    def ofPacket(packet=[]):
        return MsgPiece(packet=packet)
    ...
    def __str__(self):
        try:
            return (
                ...
                f"buffer_length: {len(self.buffer)}"
                f")"
            )
    ...
    def __eq__(self, other):
        ...
        return (
            self.identifier_file == other.identifier_file and
